...
if(filter_input(INPUT_POST, 'submit-single') == 'submit-single') {
    ...
    if (!empty(filter_input(INPUT_POST, 'cikkcim'))) {
        ...
        if (!empty(filter_input(INPUT_POST, 'cikktartalom'))) {
            //Tartalom
            $cikkTartalom = trim(filter_input(INPUT_POST, 'cikktartalom'));
            $cikkTartalom = str_replace("'","\'",$cikkTartalom);
        ...
        $kiemeltKep = 'slide_alapertelmezett.jpg';
        $dobozKep = 'thumb_alapertelmezett.jpg';
        //alapértelmezett cikkSorrend:
        $cikkSorrend = 9999;
        //összekészítjük és futtatjuk az INSERT-et.
        $result = mysqli_query($connGWP, "INSERT INTO gw_sal_content(cikkCim, cikkLeiras, cikkTartalom, cikkURL, kiemeltKep, dobozKep, sectionID, kiemeltMainSlider, kiemeltCikk, cikkSorrend, timeCreated, timeUpdated) VALUES ('$cikkCim','$cikkLeiras','$cikkTartalom','$keresoNev','$kiemeltKep', '$dobozKep','$section','$kiemelt','$kiemeltCikk','$cikkSorrend','$timeCreated','$timeUpdated')") or die(mysqli_error($connGWP));
        if ($result) {
            ...
            sleep(1);
            $cikkAzQ = mysqli_query($connGWP, "SELECT ID FROM gw_sal_content WHERE timeCreated='$timeCreated'");
            $cikkAzR = mysqli_fetch_array($cikkAzQ);
            $cikkAz = $cikkAzR['ID'];
            if ($cikkAz != NULL) {
                //Ha megvan az ID, akkor feltöltöm a SEO táblát és a képet is
                if (mysqli_query($connGWP, "INSERT INTO gw_sal_seo (keywords,description,robots,seoID,oldalID) VALUES ('$keywords','$description','$robots','$secID','$cikkAz')")) {
                ...
                $_SESSION['error'] = 'Hiba a mentés során. Nem sikerült beszúrni a SEO adatokat. Ok: hiányzó cikk ID.';
            }
            header("location: ".EDITSINGLE."&ID=".$cikkAz);
        } else {
            $_SESSION['error'] = 'Hiba a mentés során. Hibakód: 01010';
        }
    }
}
//A form
                    ...
                    <div class="mdl-card__supporting-text">
                        ...
                        <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
                            ...
                            <ul class="demo-list-item mdl-list cat-list">
                                <?php
                                $secQuery = mysqli_query($connGWP,"SELECT ID,secCim FROM gw_sal_sections LIMIT 10");
          ...
          <label class="demo-list-radio mdl-radio mdl-js-radio mdl-js-ripple-effect" for="section'.$secID.'">
            <input type="radio" id="section'.$secID.'" class="mdl-radio__button" name="section" value="'.$secID.'" '.$checked.'/>
                ...
                <div class="mdl-cell mdl-cell--10-col mdl-cell--1-offset-desktop mdl-card mdl-shadow--2dp">
                    ...
                    <div>
                        <div class="mdl-card__supporting-text">
                            <input type="file" name="kep" id="kep" style="display: none" required/>
                        ...
                        <div class="mdl-card__supporting-text">
                            <?php echo messages(); ?>
                            <p>Az elvárt tulajdonságok:<br/> fájltípus: jpg<?php
                ...
                <button id="add" type="button" onclick="location.href='../../../index.php'" style="bottom: 92px" name="submit-single" class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--colored mdl-color--red-500" value="back">
